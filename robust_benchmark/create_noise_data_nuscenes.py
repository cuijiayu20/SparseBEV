...
def create_noise_nuscenes_info_file(root_path, version):
    ...
    # 多级别外参噪声配置
    # L0: 干净基线（无噪声）
    # L1: 0.5°, 0.3cm - 标准在线标定误差
    # L2: 1.0°, 0.5cm - 轻度漂移
    # L3: 2.0°, 1.0cm - 中度漂移（常用测试点）
    # L4: 5.0°, 2.0cm - 严重失准（压力测试）
    extrinsics_noise_levels = {
        'L1': dict(r=(0.3, 0.5), t=(0.2*0.01, 0.3*0.01)),    # ~0.5°, ~0.3cm
        'L2': dict(r=(0.8, 1.0), t=(0.4*0.01, 0.5*0.01)),    # ~1.0°, ~0.5cm
        'L3': dict(r=(1.5, 2.0), t=(0.8*0.01, 1.0*0.01)),    # ~2.0°, ~1.0cm
        'L4': dict(r=(4.0, 5.0), t=(1.5*0.01, 2.0*0.01)),    # ~5.0°, ~2.0cm
    }
    
    # 使用 L3 作为默认级别（兼容旧代码）
    camera_extrinsics_noise = extrinsics_noise_levels['L3']

    noise_info = nuscenes_infos_gatherer_noise_val.gather(
        frames_drop_ratio_list, 
        camera_extrinsics_noise,
        extrinsics_noise_levels=extrinsics_noise_levels
    )
    
    print('Generate mmdet info.')
    mmdet_info_path = osp.join(root_path, f'nuscenes2d_temporal_infos_val.pkl')
    ...
    # 保存噪声级别配置到 pkl
    noise_info['extrinsics_noise_levels'] = extrinsics_noise_levels
    
    filename = osp.join(root_path, f'nuscenes_infos_val_with_noise.pkl')

    print(f'nuScenes noisy info val file is saved to {filename}')
    print(f'Extrinsics noise levels included: {list(extrinsics_noise_levels.keys())}')
...
class NuScenesNoiseInfoGatherer:
    ...
    def gather(self, frames_drop_ratio_list, camera_extrinsics_noise, extrinsics_noise_levels=None):
        ...
        print('[INFO] Frame drop generation SKIPPED (disabled)')
            
        
        # random add noise for camera extrinsics - 多级别支持
        if extrinsics_noise_levels:
            # 为每个级别生成噪声
            print(f'Generating multi-level extrinsics noise: {list(extrinsics_noise_levels.keys())}')
            for level_name, level_noise in extrinsics_noise_levels.items():
                set_seed(0)
                print(f'  Level {level_name}: rotation={level_noise["r"]}, translation={level_noise["t"]}')
                self.camera_extrinsics_gather(level_noise, val_nusc_infos_camera, level_name=level_name)
        else:
            # 兼容旧代码：只生成单个级别
            set_seed(0)
    ...
    def camera_extrinsics_gather(self,
                                 ...
                                 level_name=None):
        """
        Generate extrinsics noise for cameras.
        
        Args:
            camera_extrinsics_noise: dict with 'r' and 't' noise ranges
            val_nusc_infos_camera: camera info dict to update
            level_name: optional level name (e.g., 'L1', 'L2') for multi-level support
                       If provided, noise keys will be prefixed with level name
        """
        ...
        for sample in mmcv.track_iter_progress(self.nusc.sample):
            ...
            for cam in camera_types:
                ...
                # 确定噪声键名前缀
                if level_name:
                    single_name = f'{level_name}_single'
                    all_name = f'{level_name}_all'
                else:
                    single_name = 'single'
                    all_name = 'all'
                
                cam_info = self.obtain_noise_sensor2top(cam_token, l2e_t, l2e_r_mat,
                                            e2g_t, e2g_r_mat, noise_r, noise_t, noise_name=single_name)
                cam_info.update(self.obtain_noise_sensor2top(cam_token, l2e_t, l2e_r_mat,
                                            e2g_t, e2g_r_mat, noise_r_all, noise_t_all, noise_name=all_name))
                
                # 使用 L3 作为默认旧格式的噪声（兼容不指定 level 的情况）
                if level_name == 'L3':
                    # 同时生成旧格式键名以保持向后兼容
                    cam_info.update(self.obtain_noise_sensor2top(cam_token, l2e_t, l2e_r_mat,
                ...
                # 只在无级别名或首次调用时获取原始变换
                if not level_name or level_name == 'L1':
